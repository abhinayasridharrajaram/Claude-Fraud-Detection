...
if len(all_run_dirs) >= 2:
    ...
    if prev_parquet.exists():
        ...
        for col in numeric_cols:
            ...
            # KS test on samples (full column)
            curr_vals = df[col].dropna().values
            # Load previous column values from the CSV is not stored; use stat approx.
            # We cannot reconstruct the previous distribution from summary stats alone,
            # so we skip KS and note that only drift score is available without raw data.
            ks_stat = float("nan")
            ks_pval = float("nan")
            ...
            # KS requires raw previous data; mark as skipped
        ...
        print(f"[validate_data] No feature_stats.parquet in {prev_run_dir}, skipping drift.")
else:
